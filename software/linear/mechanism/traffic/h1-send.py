...
def main():
    data = list(csv.reader(open("./batched_unsw_5000_test.csv"), delimiter='\t'))
    result = numpy.array(data).astype("float")

    # Creates packets by using data from the CSV file
    import time
    for y in range(0, MAX_PKT_COUNT):
        z = [c_to_fix(x) for x in result[y]]
        k = int(result[y][8])
        pkt = Ether(src="00:00:00:00:00:01",dst="00:00:00:00:00:02")/IP(len=80, 
            src="10.0.0.1", dst="10.0.0.2")/FeatureHeader(field0=z[1], field1=z[2], field2=z[3], field3=z[4], field4=z[5], field5=z[6], field6=z[7], label=k)
